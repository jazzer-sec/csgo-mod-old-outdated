...
// Public-domain font8x8_basic (Daniel Hepper), ASCII 0x20..0x7F.
...
void Render2D::Text(float x, float y, const std::string& s, Color c, int scale)
{
    ...
    for (char ch : s) {
        ...
        const unsigned char* g = kFont8x8[uc - 0x20];
        for (int row = 0; row < 8; ++row) {
            unsigned char bits = g[row];
            for (int col = 0; col < 8; ++col) {
                if (bits & (1 << col)) {
                    float bx = penx + col * scale;
                    float by = y + row * scale;
                    for (int sy = 0; sy < scale; ++sy)
                        for (int sx = 0; sx < scale; ++sx)
                            Blend((int)(bx + sx), (int)(by + sy), c, 1.f);
                }
            }
        }
        penx += 6 * scale; // 5px glyph + 1px gap
    }
}
